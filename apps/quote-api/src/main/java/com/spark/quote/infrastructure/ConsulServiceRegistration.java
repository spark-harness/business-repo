...
@ConditionalOnProperty(prefix = "spark.quote.consul", name = "enabled", havingValue = "true")
public class ConsulServiceRegistration implements ApplicationRunner, RuntimeDependencyProbe {
    ...
    @Autowired
    public ConsulServiceRegistration(QuoteProperties properties, ObjectMapper objectMapper) {
        this(properties, URI.create(properties.getConsul().getUrl()), HttpClient.newHttpClient(), objectMapper);
    }

    ConsulServiceRegistration(QuoteProperties properties, URI consulUrl) {
        this(properties, consulUrl, HttpClient.newHttpClient(), new ObjectMapper());
    }

    private ConsulServiceRegistration(
            QuoteProperties properties, URI consulUrl, HttpClient httpClient, ObjectMapper objectMapper) {
    ...
    ServiceRegistrationRequest buildRequest() {
        QuoteProperties.Consul consul = properties.getConsul();
    ...
    private String healthCheckUrl(QuoteProperties.Consul consul) {
